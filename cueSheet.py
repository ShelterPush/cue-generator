...
print('Do you want an explanation on how to include multiple file formats (e.g. all flac and wav files)?')
extenYN = yesNo()
if extenYN == 'y':
    print('To search for multiple file types, enclose the first letter of each format in square brackets,')
    print(' then the second letter of each format, and so on. If the number of letters does not match,')
    print(' add an asterisk to the end. For example, if you wanted to look for both flac and wav files,')
    print(' you would type:')
    print('[fw][la][av]*')
    print('Please note that this would also look for .wlv files, .faa files, and so on.')
    print()
print('Which file extension(s) are you looking for (e.g. flac, mp3, wav, etc.)? Do not include the . symbol.')
...
for i in range(len(trackFile)):
    ...
    trackDict[strNumber] = trackFile[i]
'''else:
...
# Sort the track name dictionary
...
for number in sortTitles.keys():
    print('Track %s - %s by %s'%(number,sortTitles[number],artistDict[number]))#sortArtists[number]))
print('If this is incorrect, please either edit the .cue sheet after completion or exit the script now.')
try:
    print('Press Enter to continue or keyboard interrupt (Ctrl+C) to quit.')
    input()
...
cueFile.write('PERFORMER "' + albumArtist + '"\n')
cueFile.write('TITLE "' + albumTitle + '"\n')
# Important variables right now are sortTitles, artistDict, and trackDict
...
for number in trackNum:
    trackArtist.append(artistDict[number])
    trackFilename.append(trackDict[number])
for i in range(len(trackFile)):
    cueFile.write('FILE "' + trackFilename[i] + '" WAVE\n')
